...
class SelectTargetPersonalityScreen extends ConsumerWidget {
  const SelectTargetPersonalityScreen({super.key});
                  ...
                  AppValues.personalities
                      .map(
                        (personality) => getPersonalityItem(
                          ...
                          contains: ref.watch(targetPersonalityControllerProvider).contains(personality),
                        ),
                      )
                      .toList(),
            ),
  ...
  Widget getPersonalityItem(BuildContext context, {required String personality, required bool contains}) {
    return GestureDetector(
      onTap: () {
        if (contains) {
          LocalUtil.showMessage(context, message: "이미 선택된 성격입니다.");
          return;
        }
