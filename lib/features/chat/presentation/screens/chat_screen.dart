...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  void initialize() async {
    ...
    var result = await ref.read(chatControllerProvider.notifier).read(readChatDto);
  ...
  void messageInitialize() {
    ...
    _messageSubscription = ref.read(messageControllerProvider.notifier).readAll(_chat!).listen((snapshot) {
  ...
  void sendTargetMessage() async {
    ...
    try {
      ...
      var result = await ref.read(langchainDatasourceProvider).nextTargetMessage(langchainDto);
      if (!mounted) return;
      var message = Message(
        chatId: _chat!.id,
        senderId: target.id,
  ...
  Future<void> loadMoreMessage() async {
    ...
    var moreMessages = await ref.read(messageControllerProvider.notifier).readMore(readMoreMessageDto);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          ref.watch(targetControllerProvider).target!.name,
          style: TextStyle(
            fontSize: 18,
            height: 28 / 18,
            letterSpacing: -0.5,
        ...
        actions: [
          GestureDetector(
            onTap: () async {
              ...
              if (result != null) {
                var reportInformation = ReportEntity.of(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return OtherMessageBubble(
                        ...
                        target: ref.read(targetControllerProvider).target!,
                      );
                    },
                  ),
                );
