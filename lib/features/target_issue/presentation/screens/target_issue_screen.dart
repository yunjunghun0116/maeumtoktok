...
class TargetIssueScreen extends ConsumerStatefulWidget {
  const TargetIssueScreen({super.key});

  @override
  ConsumerState<TargetIssueScreen> createState() => _TargetIssueScreenState();
}
...
class _TargetIssueScreenState extends ConsumerState<TargetIssueScreen> {
  void _createDialog(IssueType issueType) async {
    ...
    await ref.read(targetIssueControllerProvider.notifier).create(createIssueDto);
  ...
  void _updateDialog(TargetIssue issue) async {
    ...
    if (!mounted) return;
    var target = ref.read(targetControllerProvider).target!;
    var controller = ref.read(targetIssueControllerProvider.notifier);
    if (result.isEmpty) {
  ...
  List<TargetIssue> get issuesList {
    var issueController = ref.read(targetIssueControllerProvider);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          '${ref.watch(targetControllerProvider).target!.name} 와(과)의 기억',
          style: TextStyle(
            fontSize: 18,
            height: 28 / 18,
            letterSpacing: -0.5,
      ...
      body: Column(
        children: [
          Container(
            ...
            alignment: Alignment.center,
            child: Text(
              "${ref.watch(targetControllerProvider).target!.name} 와(과)의 기억은 자세하게 입력할 수록\n상대방을 더 잘 반영하여 채팅할 수 있습니다.",
